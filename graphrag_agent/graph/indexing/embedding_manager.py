...
class EmbeddingManager:
    ...
    def setup_community_index(self):
        """自动检测维度并设置社区向量索引"""
        try:
            dimension = len(self.embeddings_model.embed_query("test"))
            query = f"""
            CREATE VECTOR INDEX community_vector_index IF NOT EXISTS
            FOR (com:`__Community__`) ON (com.embedding)
            OPTIONS {{indexConfig: {{
            ...
            self.console.print(f"[green]社区向量索引设置完成，维度: {dimension}[/green]")
        except Exception as e:
            self.console.print(f"[red]创建社区索引失败: {e}[/red]")
    ...
    def update_community_embeddings(self) -> int:
        """为社区摘要生成 Embedding 并自动创建索引"""
        # 1. 自动探测维度并创建索引
        dimension = len(self.embeddings_model.embed_query("test"))
        self.graph.query(f"""
            CREATE VECTOR INDEX community_vector_index IF NOT EXISTS
        ...
        communities = self.graph.query("""
            MATCH (com:`__Community__`)
            WHERE com.embedding IS NULL AND com.summary IS NOT NULL
            RETURN elementId(com) AS neo4j_id, com.summary AS text
        """)

        if not communities: return 0

        # 3. 批量计算并更新 (复用类内的 _compute_embeddings_batch)
        texts = [c["text"] for c in communities]
        embeddings = self._compute_embeddings_batch(texts)

        updates = [{"neo4j_id": communities[i]["neo4j_id"], "emb": emb}
                   for i, emb in enumerate(embeddings) if emb]

        self.graph.query("""
            UNWIND $updates AS update
            MATCH (com) WHERE elementId(com) = update.neo4j_id
            SET com.embedding = update.emb
        """, {"updates": updates})
    ...
    def process(self, entity_limit: int = 500, chunk_limit: int = 500) -> Dict[str, Any]:
        """
        执行完整的Embedding更新流程（包含实体、文本块和社区摘要）
        """
        start_time = time.time()

        try:
            # 1. 自动创建/校验社区向量索引
            self.setup_community_vector_index()

            # 2. 设置基础 Embedding 追踪
            self.setup_embedding_tracking()

            # 3. 更新实体 Embedding (用于 A 路对齐降级)
            entity_count = self.update_entity_embeddings(limit=entity_limit)

            # 4. 更新文本块 Embedding (用于 B 路直连检索)
            chunk_count = self.update_chunk_embeddings(limit=chunk_limit)

            # 5. 【关键缺失】：更新社区摘要 Embedding (用于 B 路社区检索)
            # 假设你在 EmbeddingManager 中已经按照之前的建议添加了此方法
            comm_count = self.update_community_embeddings()

            # 记录统计信息
            self.stats["community_updates"] = comm_count
            ...
            return {
                ...
                "community_updates": comm_count,  # 增加此项
                "total_updates": entity_count + chunk_count + comm_count,  # 修正总数

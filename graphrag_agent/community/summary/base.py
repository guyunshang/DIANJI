...
class BaseCommunityRanker:
    ...
    def cleanup_redundant_communities(self) -> None:
        """物理删除冗余的单分叉高级社区，并重新连接层级"""
        print("清理冗余的单分叉高级社区...")
        # 该 Cypher 寻找只有一个子节点的高级社区，将其删除，并让子节点直接连接到祖父节点
        cleanup_query = """
                MATCH (child)-[:IN_COMMUNITY]->(p:__Community__)
                WHERE p.level > 0
                WITH p, collect(child) AS children
                WHERE size(children) = 1
                WITH p, children[0] AS singleChild
                // 查找祖父节点，维护层级链
                OPTIONAL MATCH (p)-[:IN_COMMUNITY]->(gp:__Community__)
                FOREACH (x IN CASE WHEN gp IS NOT NULL THEN [1] ELSE [] END |
                    MERGE (singleChild)-[:IN_COMMUNITY]->(gp)
                )
                // 执行物理删除
                DETACH DELETE p
                """
    ...
    def calculate_ranks(self) -> None:
        ...
        try:
            # 权重公式：Rank = 关联文本块数量 + 实体数量
            # 这样实体越多的社区，即使被引用的 Chunk 不多，也会排在前面
            result = self.graph.query("""
                    MATCH (c:`__Community__`)<-[:IN_COMMUNITY*]-(e:`__Entity__`)
                    OPTIONAL MATCH (e)<-[:MENTIONS]-(d:`__Chunk__`)
                    WITH c, count(distinct d) AS chunkCount, count(distinct e) AS entityCount
                    SET c.community_rank = chunkCount + entityCount 
...
class BaseSummarizer(ABC):
    ...
    def process_communities(self) -> List[Dict]:
        ...
        try:
            # 先清理冗余社区，再计算权重 ---
            self.ranker.cleanup_redundant_communities()
            # 计算社区权重

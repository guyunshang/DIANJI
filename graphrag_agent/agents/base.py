...
class BaseAgent(ABC):
    ...
    def __init__(self, cache_dir="./cache", memory_only=True):
        """
        初始化搜索工具
    ...
    def _render_tool_definitions(self) -> str:
        """生成工具定义的文本描述 (用于不支持 bind_tools 的模型)"""
        tool_descs = []
        for tool in self.tools:
            # 简单提取参数 schema
            args_desc = json.dumps(tool.args, ensure_ascii=False)
            tool_descs.append(f"### 工具: {tool.name}\n- 描述: {tool.description}\n- 参数格式(JSON): {args_desc}")
        return "\n\n".join(tool_descs)

    # --- 辅助方法：手动处理工具调用 ---
    def _fallback_tool_invoke(self, messages):
        """当 bind_tools 失败时的回退处理逻辑"""
        # 1. 构建包含工具描述的 System Prompt
        tool_desc_str = self._render_tool_definitions()

        # 专门针对 DeepSeek-R1 的提示词，兼容 <think> 标签
        system_instruction = f"""你是一个智能助手。你可以使用以下工具来辅助回答问题：
        {tool_desc_str}
        【绝对规则】：
        1.你必须优先使用 local_search_structured 工具来查询知识库。
        2. 严禁利用你自身的预训练知识回答问题。
        3. 你必须且只能通过调用工具获取信息。
        4. 如果需要使用工具，请在思考结束后，以 JSON 格式输出工具调用请求。
        5. JSON 格式必须包含 "tool" (工具名) 和 "parameters" (参数字典)。
        6. 示例格式：
        ```json
        {{
            "tool": "search_tool",
        ...
        7. 如果你认为不需要工具或工具返回信息不足，请在回答中明确说明“在提供的专业文档中未找到相关记录”。"""
        enhanced_messages = [HumanMessage(content=system_instruction)] + messages  # 这里选择构建一个新的消息列表
        # 2. 调用 LLM (不带 bind_tools)
        ...
        if not json_matches:
            # 尝试匹配裸 JSON (以防模型没写 code block)
            json_matches = re.findall(r'(\{\s*"tool":\s*".*?"\s*,\s*"parameters":\s*\{.*?\}\s*\})', content,
                                      re.DOTALL)
            ...
            try:
                ...
                if "tool" in data and "parameters" in data:
                    tool_calls.append({
                        ...
                        "id": f"call_{int(time.time() * 1000)}"  # 伪造一个 ID
                    })
            except Exception as e:
                print(f"[Warn] 解析工具 JSON 失败: {e}")

        # 4. 如果解析到了工具，构造带 tool_calls 的 AIMessage
        if tool_calls:
            # 移除 JSON 部分，保留思考过程，以免重复展示
            # (可选) clean_content = re.sub(r'```json.*?```', '', content, flags=re.DOTALL).strip()
    ...
    def _agent_node(self, state):
        ...
        try:
            ...
            error_str = str(e)
            # 捕获 Ollama 的 400 错误 (does not support tools)
            if "400" in error_str and "support tools" in error_str:
                print(f"\n[Info] 检测到模型不支持原生工具调用 (DeepSeek-R1)，切换到 Prompt 模拟模式...")
                response = self._fallback_tool_invoke(messages)
            else:
                # 如果是其他错误，直接抛出
                raise e
